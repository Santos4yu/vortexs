...
function renderMoneylineResearch(games) {
  ...
  const signed = (value) => { const n = Number(value || 0) * factorDirection; return `${n >= 0 ? "+" : ""}${n.toFixed(1)}%`; };
  ...
  report.insertAdjacentHTML("beforeend", `<section class="ml-model-card"><div class="ml-model-head"><div><span>MODEL BREAKDOWN</span><strong>${escapeHtml(game.game_archetype || "BALANCED MATCHUP")}</strong></div><div><span>CONFIDENCE</span><strong>${moneylineValue(game.confidence_score)}/100 · ${escapeHtml(game.confidence_band || "—")}</strong></div><div><span>VOLATILITY</span><strong class="vol-${String(game.volatility || "LOW").toLowerCase()}">${escapeHtml(game.volatility || "—")} ${moneylineValue(game.volatility_score)}/100</strong></div></div><div class="ml-factor-grid">${[["Pitching", "pitching"], ["Offense", "offense"], ["Bullpen", "bullpen"], ["Team form", "team_form"], ["Venue / H2H", "venue"], ["Market value", "market_value"]].map(([label, key]) => `<div><span>${label}</span><strong class="${Number(factorScores[key] || 0) * factorDirection >= 0 ? "plus" : "minus"}">${signed(factorScores[key])}</strong></div>`).join("")}</div><div class="ml-action-row"><div><span>FAIR ODDS</span><strong>${moneylineValue(game.fair_odds)}</strong></div><div><span>STATUS</span><strong>${escapeHtml(game.tier || "PASS")}</strong></div><p>${(game.volatility_reasons || []).length ? escapeHtml(game.volatility_reasons.join(" · ")) : "Stable conditions based on available data."}</p></div></section>`);
